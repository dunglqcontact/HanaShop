        ...
        try {
            ...
            if (currentRole != null) {
                if (currentRole.getRoleName().equals("Customer")) {
                    ...
                    String status = request.getParameter("txtProductStatus");
                    if (status.equalsIgnoreCase("true")) {
                        ProductDTO product = new ProductDTO(productID, productName, price, 0);
                        OrderDetailDTO orderDetail = (OrderDetailDTO) session.getAttribute("ORDER_DETAIL");
                        if (orderDetail == null) {
                            orderDetail = new OrderDetailDTO(null);
                        }
                        orderDetail.add(product);
                        session.setAttribute("ORDER_DETAIL", orderDetail);
                        session.setAttribute("PRODUCT_LIST", orderDetail.getCart().values());
                        request.setAttribute("message", "Add product" + productName + " to cart successful");
                        url = SUCCESS;
                    }else{
                        request.setAttribute("STATUS_FAIL", "Status is false, cannot add");
                        url = SUCCESS;
                    }
                } else {
                    url = LOGIN;
