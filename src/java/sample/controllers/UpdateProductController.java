...
public class UpdateProductController extends HttpServlet {
    ...
    private static final String ERROR = "updateProductPage.jsp";
            ...
            if (currentRole != null) {
                if (currentRole.getRoleName().equals("Admin")) {
                    ...
                    if (statusStr.isEmpty()) {
                        error.setStatusError("Status is not choosing");
                        check = false;
                    }
                    if (check) {
                        UserDTO currentUser = (UserDTO) session.getAttribute("CURRENT_USER");
                        ProductDAO productDAO = new ProductDAO();
                        productDAO.updateProduct(productID, productName, description, price, dateobj, categoryID, quantity, imagePart, status, currentUser.getUserID(), dateobj);
                        url = SUCCESS;
                    } else {
